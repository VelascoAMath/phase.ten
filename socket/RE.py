...
class RE:
            ...
            for i, phase_component in enumerate(self.phase.split("+")):
                match phase_component[0]:
                    case "R":
                        ...
                        (start_state, final_state_set) = _strictRun(
                            int(phase_component[1:])
                        )
                        state_collection.append((start_state, final_state_set))
                    case "C":
                        self.len += int(phase_component[1:])
                        (start_state, final_state_set) = _strictColor(
                            int(phase_component[1:])
                        )
                        state_collection.append((start_state, final_state_set))
                    case "S":
                        self.len += int(phase_component[1:])
                        (start_state, final_state_set) = _strictSet(
                            int(phase_component[1:])
                        )
                        state_collection.append((start_state, final_state_set))
                    case _:
                        raise Exception(
                            f"{phase_component} is not a valid phase component in {self.phase}!"
                        )
    ...
    def score(self, card_list: CardCollection) -> int:
        ...
        class BNBState:
                ...
                if not self.deck:
                    return [
                        BNBState(
                            deck=self.deck[:],
                            hand=self.hand[:],
                            curr_re_state=self.curr_re_state,
                ...
                for i, card in enumerate(self.deck):
                    if self.curr_re_state.is_final:
                        ...
                        for (
                            nextCard,
                            nextState,
                        ) in self.curr_re_state.cardTransition.items():
                            if nextCard == card:
                                deck = self.deck[:]
                                hand = self.hand[:]
                        ...
                        for (
                            nextColor,
                            nextState,
                        ) in self.curr_re_state.colorTransition.items():
                        ...
                        for (
                            nextRank,
                            nextState,
                        ) in self.curr_re_state.rankTransition.items():
            ...
            def __str__(self):
                return (
                    f"BNBState(deck={[str(card) for card in self.deck]}, hand={[str(card) for card in self.hand]}, "
        ...
        candidate_set.insert(
            BNBState(deck=card_list, phase_len=self.len, curr_re_state=self.startState)
        )
    ...
    def isSubsetAccepted(
        self, card_list: CardCollection
    ) -> tuple[bool, CardCollection]:
        """
...
def main():
    ...
    print(
        rr.isFullyAccepted(
            [
                Card.from_string(c)
                for c in "W B3 W R7 B7 W G2 W B4 B5 Y6 W R8 Y8 Y3 W W".split(" ")
            ]
        )
    )
    rr.to_graphviz()
    ...
    print(
        rr.score(
            CardCollection(
                [Card.from_string(c) for c in "G4 W W B10 W W G11 W Y2 S".split(" ")]
            )
        )
    )
