...
class RE:
    ...
    def __post_init__(self):
        ...
        if m:
            print(m.group(1))
            (self.startState, _) = _strictColor(int(m.group(1)))
            has_match = True
        ...
        if m:
            print(m.group(1))
            (self.startState, _) = _strictRun(int(m.group(1)))
            has_match = True
        ...
        if m:
            print(m.group(1))
            (self.startState, _) = _strictSet(int(m.group(1)))
            has_match = True
...
def main():
    rr = RE("S4")
    print(rr.isFullyAccepted([Card.from_string(c) for c in "W W W R7".split(" ")]))
    rr.to_graphviz()
